...
import com.phantoms.phantomsbackend.common.utils.FF14NewsUtils;
...
@Component
public class FF14NewsScheduler {
    ...
    private List<String> getNewsGroupIds() {
        String groupIds = systemConfigService.getString("ff14.news.group-ids", "");
        if (groupIds.isEmpty()) {
            return List.of();
        }
    ...
    public void fetchAndSendFF14News() {
        ...
        try {
            ...
            logger.info("获取FF14新闻列表结束，耗时 {}ms", System.currentTimeMillis() - start);
        }
    }
