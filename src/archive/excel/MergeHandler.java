...
public class MergeHandler extends AbstractMergeStrategy {
     ...
     * @param columns
     */
    public MergeHandler(boolean alikeColumn, boolean alikeRow, int rowIndex, Set<Integer> columns, Set<Integer> rows){
     ...
     * @param rowIndex
     */
    public MergeHandler(boolean alikeColumn, int rowIndex){
        this(alikeColumn, false, rowIndex, new HashSet<>(),new HashSet<>());
    }
     ...
     * @param columns
     */
    public MergeHandler(boolean alikeRow, int rowIndex, Set<Integer> columns){
        this(false, alikeRow, rowIndex, columns,new HashSet<>());
    }
    ...
    protected void merge(Sheet sheet, Cell cell, Head head, Integer integer) {
        ...
        // 列合并
        if (alikeColumn && columnId > 0&& rows.contains(rowId)) {
            ...
            Cell preRowCell = sheet.getRow(rowId==0?1:rowId-1).getCell(columnId);
            String preRowCellVal = this.getCellVal(preRowCell);
            if (null != currentCellVal && null != preCellVal && !StringUtils.isEmpty(preCellVal) && !StringUtils.isEmpty(currentCellVal)) {
            ...
            if (null != currentCellVal && null != preRowCellVal && !StringUtils.isEmpty(preRowCellVal)&& !StringUtils.isEmpty(currentCellVal)) {
                // 当前单元格内容与上一行单元格内容相等,进行合并处理
                if (preRowCellVal.equals(currentCellVal)&& rowId>0&&rows.contains(rowId-1)) {
